...
viewTestDiff <- function(
  ...
  if (interactive) {
    ...
    results <- lapply(testnames, function(testname) {
      ...
      viewTestDiffSingle(appDir, testname, preprocess, suffix)
    })
    ...
    result <- textTestDiff(appDir, testnames, preprocess, images, suffix)
    cat(result)
    invisible(attr(result, "status", exact = TRUE))
  }
}

viewTestDiffSingle <- function(appDir = ".", testname = NULL, preprocess = NULL, suffix = NULL) {
  testDir <- findTestsDir(appDir, quiet=TRUE)
