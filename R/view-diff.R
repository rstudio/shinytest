  ...
  get_file_contents <- function(filename) {
    ...
    if (grepl("\\.json$", filename) || grepl("\\.download$", filename)) {
      if (!is.null(preprocess))
        raw_to_utf8(preprocess(filename,bin_data))
      else
...
viewTestDiffWidget <- function(appDir = ".", testname = NULL, preprocess = NULL) {
  ...
  diffviewer_widget(expected, current, preprocess=preprocess)
}
...
viewTestDiff <- function(appDir = ".", testnames = NULL,
  interactive = base::interactive(), preprocess = NULL)
{
  if (interactive) {
    ...
    results <- lapply(testnames, function(testname) {
      ...
      viewTestDiffSingle(appDir, testname, preprocess)
    })
    ...
    result <- textTestDiff(appDir, testnames, preprocess)
    cat(result)
...
viewTestDiffSingle <- function(appDir = ".", testname = NULL, preprocess = NULL) {
  ...
  withr::with_options(
    list(
      ...
      shinytest.preprocess = preprocess
    ),
    invisible(
...
textTestDiff <- function(appDir = ".", testnames = NULL, preprocess = NULL) {
  ...
  diff_results <- lapply(
    testnames,
    function(testname) {
      result <- textTestDiffSingle(appDir, testname, preprocess)
...
textTestDiffSingle <- function(appDir = ".", testname = NULL, preprocess = NULL) {
  validate_testname(appDir, testname)
  ...
  diff_files(expected_dir, current_dir, preprocess)
}
