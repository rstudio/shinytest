...
As of this writing testing Shiny documents should be done with a development version of the rmarkdown package, which can be installed using `devtools::install_github("rstudio/rmarkdown")`. It will be released in the future as rmarkdown 1.7.
...
For testing Shiny documents, only one .Rmd file is allowed per directory. This is because of the way that the test directory is structure. (Note that if you name your Shiny document index.Rmd, then when deployed on Shiny Server, the URL can be use the directory name, like http://mydomain.com/dirname, instead of http://mydomain.com/dirname/index.Rmd. When deployed on RStudio Connect or Shinyapps.io, the filename is automatically not used in the URL.)
