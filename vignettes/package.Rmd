...
In this case, you can run `recordTest()` and `testApp()` as normal. After you create and run the tests, there will be a tests/ subdirectory in the application directory that stores the test scripts and results. That directory should be added to .Rbuildignore, so that the tests are not included as part of a built package.

The .Rbuildignore file should include a line like this:
...
To use shinytest to test the application, the key is to add an app.R file that simply calls the function, and then use shinytest on that app.R. You can see the example application. In this case, the application lives at tests/testthat/helloworld/app.R, and contains the following:
