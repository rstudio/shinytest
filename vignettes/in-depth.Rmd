...
For example, when clicking a button that adds dynamic UI containing a numeric input value.  This is a two stage process. Stage 1 sets the button value and the UI is placed.  In stage 2, Shiny then recognizes there is new UI values and sets the numeric input value on `input`.  This will require two separate calls to update `output`.  `app$setInput('my_button', 1)` will return after the button value is set (Stage 1). This is too early for the numeric value to be found.  Therefore, we can call `app$waitForValue("my_number_1")` to wait until the numeric input is set.
...
numVal <- app$waitForValue('my_number_1')
