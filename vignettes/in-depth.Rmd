...
Occasionally, you may need to exclude specific objects from the snapshot, such as objects that change from run to run (e.g. timestamps). Specify the name of these objects with `exclude`:

```{r}
app$snapshot(items = list(exclude=c("a", "e1")))
```

Objects with the specified names will be excluded from input, output, and export.   
